...
Beampy.onkeydown = function (aEvent) {
  ...
  if (aEvent.keyCode == 32) {
    // space
    aEvent.preventDefault();
    $("#html_store_slide_" + this.id_slide + "-" + this.layer + " video").each(
      function () {
...
Beampy.resizebokeh = function () {
  ...
  var bkplt = $(
    "#html_store_slide_" + this.id_slide + "-" + this.layer + " #bk_resizer",
  );
...
Beampy.toggleContent = function () {
  ...
  var videos = $(
    "#html_store_slide_" + this.id_slide + "-" + this.layer + " video",
  );
...
Beampy.animatesvg = function (
  ...
  var maingroup = document.getElementById(
    "svganimate_" + slide_num + "-" + slide_layer + "_" + anim_number,
  );
  ...
  var intervalometer = setInterval(
    function () {
      display_frames();
    },
    (1000 * 1) / fps,
...
Beampy.setSlide = function (aIdx, layer) {
  ...
  if (this.id_slide <= this.slides.length - 1) {
    ...
    if (this.layer <= this.maxlayer) {
      $("section").html(
        curslide.svg_header + curslide.svg[layer] + curslide.svg_footer,
      );
      Beampy.toggleContent();
      //Pour bokeh (test if the object Bokeh exist)
      //Bokeh change the structure of it's div from 0.13 version
      var bokeh = $(
        "#html_store_slide_" + this.id_slide + "-" + this.layer + " .bk-root",
      );
...
function init() {
  ...
  window.addEventListener(
    "DOMMouseScroll",
    Beampy.onmousewheel.bind(Beampy),
    false,
  );
}
window.onload = init;
