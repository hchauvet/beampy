...
def latex2svg(latexstring, cached_preamble=None, write_tmpsvg=False):
    ...
    # The __APPS__ is generated by the beampy.__init__.py file
    dvisvgmcmd = Store.get_exec('dvisvgm')
    latexcmd = Store.get_exec('latex')
    ...
    with tempfile.NamedTemporaryFile(mode='w', prefix='beampytmp', suffix='.tex') as f:
        ...
        if cached_preamble is not None:
            cmd = f'cd {tmppath} && {latexcmd} -interaction=nonstopmode -fmt="{str(cached_preamble)}" {f.name}'
        else:
            cmd = f'cd {tmppath} && {latexcmd} -interaction=nonstopmode {f.name}'
...
def process_latex_header(texfile: object, texcommands: str):
    ...
    tex = os.popen("cd "+str(texfile.parent)+rf' && {latexcmd} -ini -interaction=nonstopmode "&latex {texfile.name}\dump"')
    tex_outputs = tex.read()
    tex.close() # close the os.popen
...
def epstopdf(eps_input_file, pdf_output_file):
    ...
    eps2pdfcmd = Store.get_exec('epstopdf')
    return check_call([eps2pdfcmd,
...
def pdf2svg(pdf_input_file, svg_output_file):
    ...
    pdf2svgcmd = Store.get_exec('pdf2svg')

    return check_call([pdf2svgcmd,
